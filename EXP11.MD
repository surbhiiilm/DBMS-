# <CENTER>
## 1. Delete those employees who joined the company before 31-dec-82 while there dept location is 'Mumbai' or 'Delhi'.
```sql
DELETE FROM EMPLOYEE
    -> WHERE HIREDATE < '1982-12-31'
    -> AND DEPTNO IN(SELECT DEPTNO FROM DEPARTMENT WHERE LOCATION IN('MUMBAI','DELHI'));
```
...
SELECT M.ENAME
    ...
    -> WHERE E.MGR = M.EMPNO);
```
## 10. Display employee names,sal,comm and net pay for those employees whose net pay are greater than or equal to any other employee salary of the company?
```sql
SELECT ENAME,SAL,COMM,
    -> (SAL + IFNULL(COMM,0)) AS NET_PAY
    -> FROM EMPLOYEE
    -> WHERE (SAL + IFNULL(COMM,0)) >= ANY (SELECT SAL
    -> FROM EMPLOYEE);
```